...
void getStatus(){
    ...
    datasend += "\", \"devicetype\" : \"gassensor\", \"typecontrol\" : \"getstatus\",  \"number_disconnect\" : \"";
...
void loop(void)
{
    ...
    if(isCanSending == true && digitalRead(PIN_GAS) == LOW && isHaveGas == false){
        Time = millis();
        isHaveGas = true;
        isCanSending = false;
    ...
    if(digitalRead(PIN_GAS) == HIGH && isHaveGas == true){
        isHaveGas = false;
        isCanSending = false;
        Time = millis();
    }
    ...
    if(millis() >= (Time + TIME_KEEP_SENDING) && digitalRead(PIN_GAS) == HIGH){
        isCanSending = true;
    }
